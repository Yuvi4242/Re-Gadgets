...
import { Phone, Briefcase, Award, FileUp } from 'lucide-react';
...
const TechnicianProfileForm = ({ onSuccess }) => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      formData.append('serviceCity', data.serviceCity);
      formData.append('pincode', data.pincode);
      formData.append('bio', data.bio || '');
      if (idFile) {
        formData.append('ownerIdProof', idFile);
      }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="space-y-1.5">
        <label className="text-sm font-medium text-slate-400 font-mono tracking-tighter uppercase text-[10px]">Upload ID Proof (PDF)</label>
        <label className="border-2 border-dashed border-slate-800 rounded-lg p-4 text-center hover:border-brandPurple transition-colors cursor-pointer group block">
          <FileUp className="w-8 h-8 text-slate-500 group-hover:text-brandPurple mx-auto mb-1 transition-colors" />
          <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest">
            {idFile ? idFile.name : 'Tap to drop or click to upload PDF'}
          </p>
          <input
            type="file"
